...
<div class="max-w-4xl mx-auto space-y-6">
    ...
    <div class="admin-surface rounded-xl admin-fade-up p-6 space-y-3">
            ...
            <div class="min-w-0">
                ...
                <div class="flex flex-wrap items-center gap-2 mt-3">
                    ...
                    <span class="px-3 py-1 rounded-full text-xs font-semibold bg-green-100 text-green-700">
                        Hired: {{ $application->hired_at->format('M d, Y') }}
                    </span>
                    @endif
                    @if($application->fired_at)
                    <span class="px-3 py-1 rounded-full text-xs font-semibold bg-red-100 text-red-700">
                        Fired: {{ $application->fired_at->format('M d, Y') }}
                    </span>
                    @endif
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-3 text-sm">
            ...
            <div>
                <div class="text-gray-500 text-xs uppercase tracking-wide">Applicant Status Timeline</div>
                <div class="font-semibold text-gray-900">
                    @if($application->hired_at && $application->fired_at)
                    Hired {{ $application->hired_at->format('M d, Y h:i A') }} and fired {{ $application->fired_at->format('M d, Y h:i A') }}
                    @elseif($application->hired_at)
                    Hired {{ $application->hired_at->format('M d, Y h:i A') }}
                    @elseif($application->fired_at)
                    Fired {{ $application->fired_at->format('M d, Y h:i A') }}
                    @else
                    No hire or fire date recorded
                    @endif
                </div>
            </div>
        </div>
        ...
        <div class="grid grid-cols-1 lg:grid-cols-2 gap-3">
            <div class="border border-gray-200 bg-white shadow-sm">
                ...
                <div class="p-4 grid grid-cols-1 gap-y-3 text-sm">
                    <div>
            ...
            <div class="border border-gray-200 bg-white shadow-sm">
                ...
                <div class="p-4 grid grid-cols-1 gap-y-3 text-sm">
                    ...
                    <div>
                        <div class="text-[11px] uppercase tracking-[0.12em] text-gray-500 mb-1">Interview Result Date</div>
                        <div class="text-sm font-medium text-gray-900">
                            @if($application->status === 'interview')
                            Interview in progress / scheduled
                            @elseif($application->hired_at)
                            Hired on {{ $application->hired_at->format('M d, Y h:i A') }}
                            @elseif($application->fired_at)
                            Fired on {{ $application->fired_at->format('M d, Y h:i A') }}
                            @else
                            N/A
                            @endif
                        </div>
        ...
        <div class="rounded-lg border border-green-200 bg-green-50 px-4 py-3 text-sm text-green-800">
            This application was hired on {{ $application->hired_at->format('F d, Y h:i A') }}. This hire date remains visible even if the applicant is later fired.
        </div>
        @elseif($application->status === 'fired' && $application->fired_at)
        <div class="rounded-lg border border-red-200 bg-red-50 px-4 py-3 text-sm text-red-800">
            This application was hired on {{ $application->hired_at ? $application->hired_at->format('F d, Y h:i A') : 'N/A' }} and fired on {{ $application->fired_at->format('F d, Y h:i A') }}.
        </div>
        @elseif($application->status === 'interview' && $interview?->scheduled_at)
        <div class="rounded-lg border border-blue-200 bg-blue-50 px-4 py-3 text-sm text-blue-800">
            Interview is scheduled for {{ $interview->scheduled_at->format('F d, Y h:i A') }}. {{ $interview->notes ? 'Notes from the employer are shown above.' : '' }}
        </div>
        @endif
